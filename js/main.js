...
const BANK = {
    ...
    POPUP: {
        GET: document.querySelector('.popup'),
    },
};
...
const popupQ1 = () => {
    return `
    <div class="popup">
            <h1>Хочешь хакнуть? Cкажи пароль</h1>
            <input class="popup-input" type="number" >
            <button>OK</button>
    </div>
    `;
};
...
BANK.BANK.addEventListener('click', () => {
    ...
    document.body.insertAdjacentHTML('beforeend', popupQ1());
});
...
function viewUpgrade(numberBank, numberUpgrade) {
    ...
    if (numberBank >= trillion) {
        let question =
            'Ты заработал трилион рублей. А это значит что ты прошел игру. Хочешь ли ты начать все заново?';
